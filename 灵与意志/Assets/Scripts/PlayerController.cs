...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isBattle && PrairieBGM.enabled)
        {
            BattleBGM.enabled = true;
            BattleBGM.volume += 0.001f;
            PrairieBGM.volume -= 0.001f;
            if (PrairieBGM.volume < 0.001)
            {
                PrairieBGM.enabled = false;
            }
        }
        else if (!isBattle && BattleBGM.enabled)
        {
            PrairieBGM.enabled = true;
            BattleBGM.volume -= 0.001f;
            PrairieBGM.volume += 0.001f;
            if (BattleBGM.volume < 0.001)
            {
    ...
    public void TakingItemController(Sprite ItemSprite, int Num)
    {
        ...
        TakingItemImage.sprite = ItemSprite;
        ItemNum.text = "+" + Num.ToString();
        System.Random random = new System.Random();
        float randomR = ((float)random.NextDouble() / 1.5f + 0.5f) / 2;
        float randomalpha = (float)random.NextDouble() * 2 * math.PI;
        float randomH = (1 + (float)random.NextDouble()) / 10;

        GameObject Ins = Instantiate(TakingItem, TakingItem.transform.position + new Vector3(randomR * math.sin(randomalpha), randomH, randomR * math.cos(randomalpha)), TakingItem.transform.rotation);
        Ins.transform.localPosition = new Vector3(Ins.transform.localPosition.x, transform.position.y + randomH + 0.7f, Ins.transform.localPosition.z);
        Ins.SetActive(true);
    ...
    public void GenerateData(int ArchiveID)
    {
        ...
        //攻击
        list.playerInfos[ArchiveID].CharacterInfos[1] = 2;
        //速度
    ...
    public void SaveData()
    {
        ...
        CharacterA.TotalCoin = TotalCoin;
    ...
    public void LoadData(int ArchiveID)
    {
        ...
        if (File.Exists(filepath))
        {
            ...
            else
            {
                ...
                TotalCoin = CharacterA.TotalCoin;
