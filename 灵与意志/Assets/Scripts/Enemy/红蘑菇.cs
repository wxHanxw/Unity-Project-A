...
public class 红蘑菇 : MonoBehaviour
{
    public GameObject DeadParticle;

    private float MovedeltaTime = 0;
    ...
    void Update()
    {
        ...
        if (enemyInfo.AttackAim != null && (enemyInfo.AttackAim.transform.position - gameObject.transform.position).magnitude < 3 && MovedeltaTime > 0)
        {
            enemyInfo.canFollowStop = false;
            MovedeltaTime -= Time.deltaTime;
            enemyInfo.NormalAttackTimes = 0;
            enemyInfo.canAttack = false;
            enemyInfo.navMeshAgent.enabled = true;
            enemyInfo.navMeshAgent.speed = enemyInfo.WalkSpeed * 4;
            enemyInfo.navMeshAgent.stoppingDistance = 1;
            Vector3 Direction = -(enemyInfo.AttackAim.transform.position - gameObject.transform.position).normalized;
            enemyInfo.navMeshAgent.destination = transform.position + Direction * 5;
        }
        else
        {
            enemyInfo.canAttack = true;
            enemyInfo.canFollowStop = true;
            FarAttackController();
        }
    ...
    {
        if (enemyInfo.isAttacking && enemyInfo.AttackAim != null)
        {
            ...
            if (enemyInfo.NormalAttackPredeltaTime > enemyInfo.NormalAttackPre)
            {
                ...
                //远程
                enemyInfo.NormalAttack.GetComponent<NormalAttackTrigger>().AttackDirection = (enemyInfo.AttackAim.transform.position + new Vector3(0, -0.3f, 0) - (transform.position + new Vector3(0, 0.1f, 0))).normalized;
                ...
                MovedeltaTime = enemyInfo.NormalAttackTimes;
            }
        }
        else
        {
