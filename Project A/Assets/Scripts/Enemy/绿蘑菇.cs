...
public class 绿蘑菇 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (enemyInfo.AttackAim != null && (enemyInfo.AttackAim.transform.position - transform.position).magnitude < 1)
        {
            transform.position = enemyInfo.AttackAim.transform.position;
        }
        ...
        if (enemyInfo.EnemyHP == 0)
        {
            if (!isDeadParticle)
            {
                ...
                for (int i = 0; i < 20; i++)
                {
                    ...
                    GameObject Ins = Instantiate(DeadParticle, transform.position + new Vector3(randomR * math.sin(randomalpha), -0.5f, randomR * math.cos(randomalpha)), transform.rotation);
                    if (i == 0)
                    {
    ...
    public void NearAttackController()
    {
        if (enemyInfo.isAttacking)
        {
            ...
            if (enemyInfo.NormalAttackPredeltaTime > enemyInfo.NormalAttackPre - 0.1f)
            {
