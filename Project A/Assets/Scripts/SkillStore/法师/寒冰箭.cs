...
public class 寒冰箭 : MonoBehaviour
{
    ...
    public float MaxExpandRange = 10f; // 石头最远能扩展的范围距离
    ...
    // 旋转速度（弧度/秒）
    private float rotationSpeed = 5f; // 修改为每秒旋转15度
    // 累计旋转角度
            ...
            if (StoneIns[i] != null)
            {
                ...
                if (Time.time - stoneActivateTime[i] < trackInterval-0.01)
                {
                    //Debug.Log($"石头{i} 追踪时间未到，当前已追踪: {Time.time - stoneActivateTime[i]:F2}秒");
                        ...
                        if(Vector3.Distance(transform.position, StoneIns[i].transform.position) <= MaxExpandRange && Vector3.Distance(nearestEnemy.position, StoneIns[i].transform.position)>= MaxExpandRange  ){
                        // 2. 计算石头围绕主角的旋转位置
                        float angle = (2 * Mathf.PI / NumofStone) * i + currentRotation; // 叠加旋转角度
                        Vector3 offset = (nearestEnemy.position - transform.position).normalized * StartRange;
                        Vector3 stonePos = new Vector3(
                            transform.position.x + offset.x,  // 主角X轴 + 偏移
                            transform.position.y,             // 保持与主角Y轴一致
                            transform.position.z + offset.z   // 主角Z轴 + 偏移
                        );
                        StoneIns[i].transform.position = stonePos; // 应用新位置
                        }else if(healCastTime> (GetComponent<SkillInfo>().Duration)/5  && Vector3.Distance(nearestEnemy.position, StoneIns[i].transform.position) < MaxExpandRange )
                            {
                                StoneIns[i].transform.position += toEnemy * FallSpeed * Time.deltaTime;
                                //Debug.Log($"石头{i} 追踪敌人，当前距离敌人: 3D={dist3D:F2}，stoneIns[i].position={StoneIns[i].transform.position},healCastTime={healCastTime:F2}");
                            }
                        // 4. 距离调试信息（保持不变）
                        ...
                        if (nearestEnemy )
                        {
                            // 计算水平面上的方向（忽略Y轴高度差，避免石头抬头/低头）
                            //Vector3 toEnemy = (enemyXZ - stonePos).normalized; // 
                            //Vector3 toEnemy = (nearestEnemy.position - StoneIns[i].transform.position).normalized;
                            
                            // 从石头当前位置指向敌人的水平方向
                            
                            // 设置石头朝向（仅XZ平面转向，保持Y轴垂直）
                            if (toEnemy != Vector3.zero)
                            {
                                StoneIns[i].transform.forward = toEnemy;
                            }
